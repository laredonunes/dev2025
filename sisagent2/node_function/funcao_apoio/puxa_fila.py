...
def armazenar_informacao_time(redis_conn, chave: str, valor, timeout=300):
    """
    Armazena uma informação (valor) no Redis sob uma chave específica, com expiração de 5 minutos.
    ...
    try:
        ...
        # Usa o comando SET com expiração de 300 segundos (5 minutos)
        redis_conn.set(chave, valor_para_armazenar, timeout)

        print(f"Sucesso: Informação salva na chave '{chave}' por 5 minutos.")
...
def write_bd_5_minutos(chave,dado):
    armazenar_informacao_time(r, chave, dado, 300)